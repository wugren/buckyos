...
可登陆用户中权限最大者（root用户日常用该账号登陆)
相比root用户,admin少了哪些权限？
...
单服务多用户:最常见，但是管理员可以随时调整用户是否能使用该app
单服务单用户：是上面情况的特例，app service还可以通过在app-id中包含owner-user-id实现隔离（一份app代码，但是用多个不同的容器启动，每个容器只为一个用户服务）

对于app开发者，应该声明自己是那种模式,来方便管理员在安装的时候选择正确的模型
1）单用户模式 
2）多用户模式
