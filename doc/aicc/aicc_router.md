...
6. **支持 request 级 overlay**：调用方可在每次 RPC 传入已经合成好的 `session_overlay`，AICC 将其覆盖到系统级路由配置之上。
...
| Request Overlay | 调用方在 RPC 前合成好的本次路由覆盖层，AICC 不维护 `session_id -> config` 状态。 |
...
5. 每次路由时都应重新基于当前 Provider 状态、价格、配额、系统配置和 request overlay 生成候选列表。
...
function route(request: AICCRequest): RouteDecision {
  const baseConfig = merge(factoryRoutingConfig, systemGlobalRoutingConfig);
  const effectiveConfig = request.session_overlay
    ? merge(baseConfig, request.session_overlay)
    : baseConfig;
  const policy = mergePolicy(effectiveConfig.policy, request.policy);
...
```text
factory/default route config < system global route config < request session_overlay
```

其中：

1. factory/default route config 来自发行版本或默认逻辑目录；
2. system global route config 来自 AICenter UI 或 system-config 中的系统级设置；
3. request `session_overlay` 来自调用方本次 RPC。
...
| 能力类型 | 默认路由粒度 | 说明 |
|---|---|---|
| LLM chat/completion | per request overlay | 应用可为同一 conversation 复用相同 overlay。 |
| Agent loop 内部 LLM 调用 | per request overlay | Agent 可按 step role 合成不同 overlay，例如 plan/code/summary。 |
| 文生图 | per request overlay | 通常 stateless，可每次重新调度。 |
| 摘要任务 | per request overlay | 长文分段摘要可由应用层复用 overlay。 |
| Embedding | per request overlay | 批处理偏好由调用方 overlay 表达。 |
...
应用开发者需要在自己发起的 request overlay 中定义默认模型策略，包括：
...
1. 重新定义本次请求的局部逻辑模型视图；
2. 在应用内部继承 app/global/conversation config，并把合成结果作为 `session_overlay` 传给 AICC；
3. 调整某个精确模型的全局权重；
...
| R-010 | 支持 request 级 overlay。 | 每次 RPC 可传入 `session_overlay` 覆盖系统级 route config。 |
...
| R-107 | 应用侧 route overlay 组合基础设施。 | SDK/强类型结构帮助应用合成 app/agent/conversation overlay。 |
...
4. request overlay 合并应无共享可变 session 状态；
...
3. route trace 不得直接包含 prompt 原文；
...
### 18.5 Request Overlay 覆盖

前置条件：

- `llm.chat` 有多个候选；
- system global config 默认选择 `provider_a`；
- request overlay 把同一逻辑目录权重调整到 `provider_b`。

操作：调用 `model = llm.chat` 并传入 `session_overlay`。

期望：

1. AICC 将 request overlay 覆盖到 system global config 之上；
2. 候选生成和调度使用覆盖后的配置；
3. 最终选择 `provider_b`；
4. 后续不传 overlay 的请求不会复用本次覆盖结果。
...
| 应用侧 overlay 长期偏向异常 Provider | 调用方持续传入变差 Provider 的高权重。 | 健康硬过滤优先于权重；允许 runtime failover；trace 明确显示过滤原因。 |
...
1. 支持 request `session_overlay`；
2. 提供强类型 overlay 数据结构；
3. 支持应用侧合成 app/agent/conversation overlay；
4. 支持多 profile 调度；
...
新版 AICC 模型路由需要从简单的“模型名映射到 Provider”升级为“逻辑模型目录 + 多 Provider 候选 + 策略过滤 + fallback + 调度评分 + request overlay + 可解释 trace”的完整体系。
...
5. **调度默认成本优先，但必须同时考虑可用性、延迟、质量、隐私和 request overlay 权重**；
6. **LLM/Agent 的行为一致性由应用层通过重复传入同一 overlay 或递进 overlay 表达**；
