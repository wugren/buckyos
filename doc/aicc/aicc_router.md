...
function route(request: AICCRequest): RouteDecision {
  const baseConfig = systemGlobalRoutingConfig;
...
同一 `session_id` 下，路由配置不是一次性 request 参数，而是 session 路由状态的一部分。AICC 应维护 `session_id -> session_config` 的状态机。session config 是基于“默认逻辑目录配置 + `system_config` 系统配置”合并结果的临时 overlay；未携带 overlay 的 session 直接使用这个系统合并结果。
...
4. 同一 session 第一次 request 如果未携带任何 session config，则使用默认逻辑目录配置和 `system_config` 系统配置的合并结果；
...
Session config 主要用于在不修改 AICC 系统配置的情况下，临时调整当前 session 的逻辑模型视图。它和 `system_config.services/aicc/settings.routing_config` 使用同一套 overlay schema：`logical_tree`、`children`、`items`、`policy`、`fallback` 等字段含义一致。
...
应用开发者需要在自己发起的 request/session config 中定义默认模型策略，包括：
...
Request 级配置不应发明独立的 override 语义，而应使用和系统全局 routing config overlay 兼容的 `SessionConfig` schema。典型能力包括：
...
- `inherit`：指定父 session config；AICC 不维护 per-user config，未指定时基于“默认逻辑目录配置 + system_config routing_config”的合并结果。
...
```text
default logical directory config < system_config routing_config < current session config < request session_config_patch
```
...
| R-013 | 支持配置化策略合并。 | 默认逻辑目录配置、`system_config` 系统配置、当前 session config 和 request patch 按优先级合并。 |
