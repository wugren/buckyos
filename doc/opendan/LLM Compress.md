...
#### 11.2.1 Tool Result 折叠

原始：

```text
ToolResult: 写入文件 /path/to/a.ts 成功，输出大量无关日志...
```
...
ToolResultCompressed:
  ...
  range: 1-500
  content_hash: sha256:...
  original_length: 500 lines
  note: 大型读取结果已折叠，必要时可重新读取
```
...
如果 tool result 包含错误堆栈、失败原因、diff、编译错误等，默认不应机械折叠，除非有专门规则保留关键信息。
