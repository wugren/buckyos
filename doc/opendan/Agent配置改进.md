...
#   - `[session.<class>.driver]`:运行期行为(keep_alive / switch_mode /
...
[session.ui.driver.on_wait]   # alias `on_wakeup`
filter     = "top"
pull_msg   = "all"
pull_event = "none"
...
| — | `[session.<class>.driver].keep_alive` | **新增**,固化"UI 永远活、Work 看状态"的差异 |
...
| — | `[session.<class>.driver.{on_init, on_behavior_switch, on_behavior_step_ob, on_wait}]` | **新增**,Driver hook point × filter × pull policy(详见 [Agent Session §8](./Agent%20Session.md)) |

> 旧 schema 直接在 `[session.<class>]` 顶层挂 `subscribe_events` / `switch_mode` /
> `keep_alive` / `inject_background_environment` / `report_delivery` 等字段,这些位置在
...
> (`on_init` / `on_behavior_switch` 当前生效,`on_input_event` 作为事件 fallback 生效,`on_input_msg` 尚未接入 worker drain)
