...
| `$session.current_todo` | `{{ session.current_todo }}` / `{{ session.current_todo.task }}` | object / null | `todos.json` 中第一个非终态 Todo；缺失或无当前 Todo 时为 `null` |
| `$session.current_todo_list` | `{{ session.current_todo_list }}` | string | `AgentSession.session_dir/todos.json` 的简表；缺失时为 `(empty)` |
...
`on_switch` 额外注入切换来源信息：

| 占位 | 来源 |
| --- | --- |
| `{{ switch.from }}` / `{{ from_behavior }}` | 切换前 behavior 名 |
| `{{ switch.to }}` | 切换后 behavior 名 |
