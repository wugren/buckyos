...
当前代码里的 `ContactMgr.resolve_did(platform, account_id, owner)` 仍会生成 `did:bns:mc-...` 形态的自动联系人 DID。该格式更接近 shadow contact DID，同时承担了“自动联系人”和“平台 endpoint”的角色。后续代码修改应把这两层拆开：二级 DID 使用 `did:msgtunnel:*`，正式联系人仍使用一级 DID，二者通过 binding 关联。
