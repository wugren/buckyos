...
`usage`:
```text
agent-memory [--root <path>] [--quiet] <init|set|remove|get|list|load|verify|compact> [...]
```

`--root` 缺省值由 env 决定（`AGENT_MEMORY_ROOT` → 不存在则用 agent_env_root 下的 `memory/`）；`OPENDAN_OWNER_USER_ID / OPENDAN_AGENT_ID / OPENDAN_SESSION_ID` 用作 actor / owner 上下文。
...
# 写入（content 走 argv 或 stdin）
agent-memory set "user.name" "Liu Zhicong" --reason "init profile"
...
# 触发加载策略 / 验证 / 压缩
agent-memory load --token-limit 800
agent-memory verify
...
```json
{
  "agent_tool_protocol": "1",
  "status": "success",
  "cmd_name": "agent-memory",
  "cmd_args": "agent-memory get user.name",
  "title": "agent-memory get user.name => ok",
...
```json
{
  ...
  "status": "success",
  "cmd_name": "agent-memory",
  "cmd_args": "agent-memory load --token-limit 800",
  "title": "agent-memory load => 5 items",
  "summary": "loaded 5 items / ~720 tokens",
  "detail": {
    "ok": true,
    "items": [ { "key": "user.name", "...": "..." } ],
    "token_estimate": 720
...
`--quiet` 开启时只输出顶层 `status` 适合 shell 集成。
...
```bash
agent-notebook create --kind project --title "buckyos beta2.2"
agent-notebook append --id nb_01HXY --reason "session note" <<EOF
...
agent-notebook hints --owner-agent did:opendan:alice
```
