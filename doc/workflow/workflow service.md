...
Workflow schedule 是 trigger 真相源，不是业务 executor。它只负责管理 `cron` / `once` / `run_every`、`next_fire_at`、misfire、fire record、pause/resume/archive，并在每次 fire 时按 schema 创建 TaskMgr subtask。
...
```json
{
  ...
  "data_template": {
    "agent_delegate": {
      "version": 1,
      "title": "scan photos",
      "purpose": "scan new photos",
      "workspace_hints": [{ "workspace_id": "ws-photos" }],
      "trigger": {
...
当前 `dcrontab remind` 映射为 `workflow.send_message` subtask。该 task type 的 executor 归属 Workflow service：它负责调用 Message Center，并把发送结果或错误写回 subtask；schedule manager 本身不直接发消息。

### 3.7 Thunk / Step 状态获取（不新增专用回执 RPC）
