...
### 5.7 支持多种 provider 和缓存，但输出信任上下文
...
- payment_policy：续期、转让、收益相关策略。

这个 API 用来回答“这个名字现在是否存在、由谁拥有、是否还能被注册或更新”。

### register_name(name, owner, options, proof)
...
### renew_name(name, duration, proof)
...
### transfer_name_owner(name, new_owner, proof)

转让名字资产 owner。

这个 API 表达的是“谁拥有这个名字资产”，不是简单换一个签名 key。它可以兼容 NFT / ERC-721 一类资产模型。

转让后：

- 新 owner 获得后续更新权。
- 当前 DID Document 应尽快更新 owner / controller / payment 信息。
- 旧版本文档和旧签名不能被抹掉，仍应按历史状态验证。

### resolve_did(did, doc_type, options) -> ResolveResult
...
provider 的使用顺序需要按上下文区分：

1. 全局名字的权威状态来自 BNS 合约或带 proof 的 Global Provider。
2. Zone 内子名字可以先查本 Zone provider，但必须能回溯到父 Zone / Owner 授权。
3. Global Provider，例如 SN，可以作为合约状态的加速、缓存或可信查询入口。
4. HTTPS / `.well-known` / DNS TXT 可以作为兼容发现路径，但返回内容必须继续验签和校验状态。

因此，Zone provider 不能任意覆盖全局 BNS 名字；它只应该解析自己被授权的命名空间。

### update_did_document(did, doc_type, document, proof, expected_seq)
...
`expected_seq` 用来避免并发覆盖和旧状态重放。

### revoke_did_document(did, doc_type, version_or_range, proof)
...
### set_did_alias(did, target_did, proof)
...
### rotate_owner_key(name, new_owner_key, proof)
...
如果名字资产 owner 是 NFT / 合约账户，那么 `rotate_owner_key` 可能体现为更新 OwnerConfig 中的 auth key，而不是改变 NFT owner。

### update_controller_policy(name, policy, proof)

更新 controller 策略。

Owner 不应该每天用最高权限 key 签所有文档。BNS 需要支持把不同权限委托给不同 controller：
...
controller policy 应该能表达 key usage、namespace、doc_type、有效期、撤销方式和多签要求。

### release_name(name, proof)
