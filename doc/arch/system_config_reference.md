...
| `users/<user_id>/settings` | `UserSettings`，包含用户类型、显示名、状态、资源池、联系方式、`is_local`、是否允许改密码等账号主数据。 | scheduler 初始化管理员；control-panel 创建/更新用户和邀请接受流程 | scheduler 用它识别用户及角色；system-config 用于特权用户文档加载；smb-service 读取用户列表。 |
| `users/<user_id>/profile` | `UserPrivateProfile`，用户私有 Profile。 | scheduler 初始化管理员；control-panel 创建/接受用户及 profile 更新流程 | Control Panel 用户资料视图；普通用户可自行读写。 |
