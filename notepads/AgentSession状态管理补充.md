...
- **`keep_alive` 字段在新架构下的语义**：
  - 当前稳定阶段：所有 session worker 都允许在 idle 后自动卸载；下次有 event/msg 到达再 `ensure_session`。
  - `keep_alive` 只作为未来热缓存 hint，不改变 session 的逻辑生命周期，也不能要求 Agent 启动时预加载 worker。
- **启动恢复策略**：
  - Agent 启动时只恢复轻量路由索引（如 UI tunnel 绑定、event subscription pattern），不启动所有非 END session worker。
  - `ensure_session` 只能在 main loop 已经决定要向某个 session 写入 pending input 后调用。
  - 空闲 worker 退出时只卸载内存态 worker，不删除磁盘 meta，也不删除仍然有效的 event subscription 路由。

### 8.4.1 END 后的 pending 写入拒绝

`END` 是 session 逻辑生命周期的终态。Agent main loop 在写入 pending input 之前必须先读取目标 session 的持久化状态：
...
这个拒绝发生在 Agent main loop，而不是 `AgentSession::enqueue_pending` 内部。这样 main loop 可以用统一方式处理拒绝（记录、丢弃 stale event、回复用户或创建新 Work Session），同时 `AgentSession` 不需要理解“这条输入为什么被路由给我”的上层策略。

拒绝规则：

- 显式 target 指向 `END` session：拒绝写入，不自动 reopen。
- 隐式路由不应选择 `END` session 作为候选。
- 如果用户想追问已结束任务，应由 UI/session 路由层创建新的 Work Session，并显式读取旧 session 历史作为上下文，而不是把新消息写回旧 session。
...
### 10.3 SelfCheck 的 timer reason schema
...
### 10.4 SelfImprove 的 budget 与任务续跑
...
命名上用 `on_wakeup` 表示“session 从 idle / waiting 状态被 pending input 唤醒后触发”。
...
def session.worker():
    ...
    user_msg = render_prompt(self.current_behavior.on_behavior_switch, llm_context_env)
...
`llm_context_env` 的 schema 是**固定的**（framework-level 决策），不是 per-config declared。完整变量契约见 [`doc/opendan/Agent Enviroment.md §3`](../doc/opendan/Agent%20Enviroment.md)，这里只列 hook point 视角下相关的 namespace：

| Namespace | 来源 |
| --- | --- |
| `input.msgs` | `pull_msg` 结果 |
| `input.events` | `pull_event` 结果 |
| `input.bg_events` | 半订阅 event 当前快照（不消费） |
| `session` | SessionMeta 投影（id / kind / title / objective / current_behavior / current_todo / ...） |
| `current_context.last_step` | step observation 阶段的 last_step_result（仅 Behavior Loop） |
| `current_context.step_history` | StepRecord history（仅 Behavior Loop） |
...
3. 只要 user message 已经成功追加进即将运行的 `LLMContext`（即将进入 `ctx.run()` / `drive_to_end()`），本轮 pull 到的 input 就从 `.meta/session.json.pending_inputs` 删除。

未被当前 hook point pull 到的 input 必须继续留在 pending queue；它们不属于本轮推理，不能因为同一批 pending 中有其它 input 被消费而被误删。

当前阶段只实现 **pull** 语义；未来如果引入 peek，peek 到的 input 只能进入 env/background context，不能加入本轮 commit-pop 集合。
...
| `<filter_name>` | 取队列中匹配该 filter 的 event（如 `timer.reminder_check` / `timer.hard_barrier` 等命名空间）|
| `all` | 取队列所有 event |
...
| 普通 Work Session | 标准初始化 | `filter=top, pull_msg=all` | `filter=top, pull_msg=all` | — |
...
- **SelfCheck Session** 是 timer 驱动的特殊 Work Session，主要负责周期性或精确时间点的检查，并通过 `send_message` 触发提醒。
