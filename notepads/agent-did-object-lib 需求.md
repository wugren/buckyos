...
Object ID 是路由输入，不等同于协议 wire endpoint。
...
| `did:*` | DID resolver input。第一版可只保留类型和路由能力，具体 DID route 由 adapter 实现。 |
| `obj://...` | 上层 semantic URI。进入 DID Object adapter 前必须解析到 Object URL 或 DID。 |
| alias | 配置内声明的短名，例如 `camera.home.cam01`。只用于路由配置，不进入 DID Object Protocol wire endpoint。 |
...
| `url_prefix` | URL 以 `pattern` 开头。只适用于 URL。 |
| `path_prefix` | 本地路径以 `pattern` 开头。只适用于 filesystem adapter 的 path 输入。 |
| `scheme` | URL / DID / obj URL 的 scheme 等于 `pattern`。 |
| `did_prefix` | DID 字符串以 `pattern` 开头。 |
| `alias_prefix` | alias 以 `pattern` 开头。 |
| `glob` | 仅支持末尾 `*` 的简单前缀 glob，不引入复杂 glob 依赖。 |

路由选择规则：

1. 先解析 `ObjectRef`，得到 `kind` 和 `normalized`。
...
3. route method 限制。
4. ObjectRef normalize：URL、DID、`obj://`、alias。
