...
- `deps` 描述参与 staging 准备或 manifest 合并的上游项目来源，不直接等同于安装包内的 `overwrite` module 规则。
- `cyfs-gateway` 不作为独立用户可见组件；打包脚本 MUST 显式处理 `deps.cyfs-gateway.source` 指向的项目。如果该依赖存在并被合入 staging/manifest，必须作为 `buckyos` service payload 的一部分进入安装包。
...
- `deps.*.source` 如果存在，必须是非空字符串；打包脚本不能再隐式扫描未声明的兄弟目录。
