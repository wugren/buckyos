...
- `obj://` 如保留，只能作为 `src/frame/agent-did-object-lib` 的 canonical semantic URI 或 resolver input；它不是 v1 wire endpoint。
...
`src/frame/agent-did-object-lib` 应依赖 DID Object Resolver。协议目标允许输入 Object URL 或 DID：
...
v1 核心协议不要求 `obj://`。如果 `src/frame/agent-did-object-lib` 继续使用：
...
- `src/frame/agent-did-object-lib` 是否继续保留 `obj://` 作为内部 semantic URI，还是直接统一为 Object URL / DID。
