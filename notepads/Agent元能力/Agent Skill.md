...
1. **Skill Source 不等于 Runtime Skill**。用户安装、系统内置、Hub 下载或 Agent 自己总结出来的内容，都只是来源；必须经过编译、拆解、分类、命名、验证，才能成为运行时 skill。
...
| Skill Compiler | 把来源文本或包拆解成 runtime skill candidate |
...
`SkillSource` 是进入系统的原始材料，不是运行时 skill。
...
`RuntimeSkill` 是 Skills Mgr 可以召回和加载的运行时 artifact。它必须有：
...
`SkillCandidate` 是尚未进入 active registry 的候选 skill。它可能来自：
...
缺少这些字段的内容只能作为 Skill Source 或 Draft Candidate，不能进入 active registry。
...
Active skill 至少需要 `static_checked`。有外部副作用的 workflow / delivery skill，进入 preferred 前必须有 `manual_verified` 或足够的 `usage_verified`。
...
9. Candidate Write
   写入 SkillCandidate，不进入 active registry。
...
3. 如果 skill 与 runtime governance 冲突，governance 优先；
...
Prompt 级“不要修改”不足够。Skills Mgr 必须在 runtime / storage 层拒绝越权写入。
...
### 12.3 Verification / Promotion APIs

```text
skill_request_eval(skill_or_candidate_id, eval_plan) -> EvalRun
skill_record_eval_result(eval_run_id, result) -> EvalResult
skill_promote(candidate_id, options) -> RuntimeSkill
skill_mark_needs_reverification(skill_id, reason) -> RuntimeSkill
```
...
新的 Skills Mgr 不需要保持旧语义兼容，但应给出清晰迁移路径：
...
| `load_skill` | `skill_view` + `skill_record_loaded` + session selected skill list |
| `unload_skill` | session selected skill list remove；不改变 active registry |
| `session.load_skills` | session-scoped explicit selected skills |
| `behavior.load_skills` | behavior-scoped recommended / pinned skill hints |
...
3. 旧 `load_skill` 不再表示“永久安装”，只表示当前 session 显式选择并读取某个 skill；
4. `behavior.load_skills` 不应默认整篇注入，只应作为该 behavior 的高优先级 hint 或 allowlist；
5. 如果某个 behavior 需要强制 skill，应声明 `pinned = true`、`protected = true` 和明确 scope；
6. Prompt Compiler 不再拼接全部可用 skill，只渲染已选择的少量 active skill；
7. 旧 skill 没有 verification 字段时，导入后状态为 `candidate` 或 `active + needs_reverification`，不能直接 preferred。

这次迁移的目标不是兼容所有旧行为，而是把旧的 prompt 片段模型收敛到带生命周期的 artifact 模型。
...
| 是否允许 agent-specific overlay patch hub skill | 允许。进入本地 Runtime 后，所有来源的 skill 在使用层面平等，最终质量由使用效果、验证和 ranking 决定。 |
