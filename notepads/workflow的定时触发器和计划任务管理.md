...
- **Workflow 管 trigger / schedule 的真相源与编排语义**：cron、timezone、misfire、next_fire_at、run creation、cursor 归属、补偿策略。
- **TaskMgr 保持被动任务总账**：镜像 schedule root task、记录每次 run / step / shard 的任务树、发布状态事件。
...
3. 支持 Workflow 定义的定时触发，也支持 OpenDAN command 这类非 Workflow target。
...
3. 不要求所有 schedule 都必须执行 Workflow DAG；简单 OpenDAN command 可以作为 schedule target。
...
- 幂等创建 schedule fire / workflow run request。
- 把 schedule 和每次执行镜像到 TaskMgr。

### 4.2 TaskMgr 镜像与任务树

每个 schedule 可以镜像为一个 TaskMgr root task：

```text
workflow/schedule: scan-new-images daily 03:00
└── workflow/run: 2026-05-27T03:00:00-07:00
    ├── workflow/step: list new images
    ├── workflow/map_shard: image A
    ├── workflow/map_shard: image B
    └── workflow/step: summarize
```

TaskMgr 中的 schedule root task 是可观察面，不是调度真相源。Workflow 更新它的状态和 `data.schedule` 摘要。
...
| kind | 说明 |
| --- | --- |
| `workflow.run` | 到点创建 Workflow Run |
| `opendan.command` | 到点创建 OpenDAN Work Session 或异步 command task |
| `service.rpc` | 到点调用标准服务 RPC，适合轻量系统维护任务 |

`workflow.run` 是最重要的主路径。`opendan.command` 用于兼容 Agent crontab 工具。
