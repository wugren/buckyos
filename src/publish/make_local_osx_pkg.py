...
def load_macos_pkg_components_from_manifest(manifest_path: Path) -> List[PublishComponent]:
    ...
    for key in component_keys:
        ...
        if not isinstance(project, dict):
            raise ValueError(f"manifest install project missing for macOS component: {key}")
        platform_cfg = (project.get("platforms", {}) or {}).get("macos")
        if not isinstance(platform_cfg, dict):
            raise ValueError(f"manifest install project '{key}' missing macos platform config")
        components.append(
            PublishComponent(
...
def build_macos_distribution_pkg(
    *,
    ...
    components = (
        load_macos_pkg_components_from_manifest(manifest_path)
        if manifest_path is not None
...
def sync_macos_scripts(
    ...
    *,
    ...
    """
    if manifest_path is not None:
        manifest = json_load_file(manifest_path)
        install_projects = manifest.get("install_projects", {}) or {}
        if not isinstance(install_projects, dict):
            raise ValueError("manifest.install_projects must be a map")
        app_keys = [
            str(key)
            for key, value in install_projects.items()
            if isinstance(value, dict) and value.get("app_key")
        ]
    else:
        data = yaml_load_file(project_yaml_path)
        apps = data.get("apps", {}) or {}
        app_keys = [str(app_key) for app_key in apps.keys()]
...
def main(argv: List[str]) -> int:
    ...
    if args.cmd == "sync-macos-scripts":
        sync_macos_scripts(
            Path(args.project),
            SRC_DIR / "publish" / "macos_pkg" / "scripts",
            manifest_path=Path(args.manifest).resolve() if args.manifest else None,
        )
