...
impl ClaudeProvider {
    pub fn new(cfg: ClaudeInstanceConfig, api_token: String) -> Result<Self> {
    ...
    async fn start_llm(
        ...
        extra.insert("provider".to_string(), Value::String("claude".to_string()));
        extra.insert(
            "model".to_string(),
...
struct ClaudeSettings {
    ...
    #[serde(default)]
    instances: Vec<SettingsClaudeInstanceConfig>,
}
...
fn parse_claude_settings(settings: &Value) -> Result<Option<ClaudeSettings>> {
    let Some(raw) = settings.get("claude").or_else(|| settings.get("anthropic")) else {
        info!("aicc claude settings missing at settings.claude");
        return Ok(None);
    };
    if raw.is_null() {
        info!("aicc claude settings present but null");
        return Ok(None);
    }

    let claude_settings = serde_json::from_value::<ClaudeSettings>(raw.clone())
        .map_err(|err| anyhow!("failed to parse claude settings: {}", err))?;
    info!(
        "aicc claude settings parsed enabled={} api_key_present={} instances={} alias_map_keys={}",
        claude_settings.enabled,
        !claude_settings.api_token.trim().is_empty(),
        claude_settings.instances.len(),
        claude_settings.alias_map.len(),
    );
    if !claude_settings.enabled {
        info!("aicc claude provider disabled by settings.claude.enabled=false");
        return Ok(None);
    }

    Ok(Some(claude_settings))
}
...
fn build_claude_instances(settings: &ClaudeSettings) -> Result<Vec<ClaudeInstanceConfig>> {
    ...
    for raw_instance in raw_instances.into_iter() {
        ...
        if models.is_empty() {
            models = normalize_model_list(parse_csv_list(DEFAULT_CLAUDE_MODELS));
        }
        if models.is_empty() {
            return Err(anyhow!(
...
pub fn register_claude_providers(center: &AIComputeCenter, settings: &Value) -> Result<usize> {
    let Some(claude_settings) = parse_claude_settings(settings)? else {
