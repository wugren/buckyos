...
const DEFAULT_TRUST_DID = ['did:web:buckyos.org', 'did:web:buckyos.ai', 'did:web:buckyos.io']
...
function makeGlobalEnvConfig(
  ...
  const activeConfigPath = path.join(targetDir, 'bin', 'node-active', 'active_config.json')
...
export async function buildUserEnv(params: OODGroupParams, envRoot: string): Promise<string> {
  const userDir = ensureDir(path.join(envRoot, params.zone_id))
  const oodNameForZone =
    params.netid !== 'lan' ? `${params.node_name}@${params.netid}` : params.node_name
...
function copyIdentityOutputs(userDir: string, nodeDir: string, targetDir: string, zoneId: string): void {
  const etcDir = ensureDir(path.join(targetDir, 'etc'))

  copyIfExists(path.join(userDir, `${zoneId}.zone.json`), path.join(etcDir, `${zoneId}.zone.json`))
  for (const name of ['start_config.json', 'node_identity.json', 'node_private_key.pem', 'node_device_config.json']) {
  ...
  copyIfExists(path.join(userDir, `${zoneId}.zone.json`), path.join(buckycliDir, 'zone_config.json'))
}

async function generateTls(zoneId: string, caName: string, etcDir: string, caDir: string): Promise<void> {
  await ensureCa(caDir, caName)
  const { certPath, keyPath } = await createCertFromCa(caDir, zoneId, etcDir, [zoneId, `*.${zoneId}`])

  fs.renameSync(certPath, path.join(etcDir, 'zone_cert.cert'))
  fs.renameSync(keyPath, path.join(etcDir, 'zone_cert_key.pem'))

  // Keep CA for trust
  copyIfExists(path.join(caDir, `${caName}_ca_cert.pem`), path.join(etcDir, 'ca.cert'))
  copyIfExists(path.join(caDir, `${caName}_ca_key.pem`), path.join(etcDir, 'ca_key.pem'))
...
async function makeIdentityFiles(targetDir: string, params: OODGroupParams, caDir: string): Promise<void> {
...
const BOOT_ENTRY_PATTERN = /^[^\S\n]*"([^"\n]+)"[^\S\n]*=[^\S\n]*"""\n([\s\S]*?)\n"""[ \t]*\n?/gm

function escapeRegExp(text: string): string {
  return text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
}

function applyDevBootTemplateOverride(targetDir: string, groupName: string): void {
  if (groupName !== 'dev' && groupName !== 'devtest_ood1') {
    return
  }

  const localBootToml = path.join(os.homedir(), '.buckycli', 'buckyos_boot.toml')
  if (!fs.existsSync(localBootToml)) {
    console.log(`skip missing dev boot override: ${localBootToml}`)
    return
  }

  const dstBootTemplate = path.join(targetDir, 'etc', 'scheduler', 'boot.template.toml')
  ...
  if (entries.length === 0) {
    console.log(`skip invalid dev boot override (no key/value found): ${localBootToml}`)
    return
  }
  ...
  console.log(`merge dev boot override into template: ${dstBootTemplate} (replaced=${replaced}, added=${added})`)
}
...
export async function makeConfigByGroupName(
  ...
  if (groupName === 'sn' || groupName === 'sn_server') {
    throw new Error('SN config generation moved to the standalone tool: deno run --allow-all src/make_sn_configs.ts')
  }
  ...
  console.log(`############ make config for group name: ${groupName} #########################`)
  ...
  makeGlobalEnvConfig(targetDir, params.web3_bridge, params.trust_did, params.force_https)
...
async function main(): Promise<void> {
  try {
    ...
    console.error('make_config.ts requires Deno >= 2.2 (or Node >= 22.13 with a loader for the websdk import)')
    Deno.exit(1)
  }
  ...
  if (positionals.length !== 1) {
    console.error('usage: make_config.ts <group> [--rootfs <dir>] [--ca <dir>]')
    console.error(`groups: ${[...Object.keys(OOD_GROUPS), 'release'].join(' | ')}`)
    Deno.exit(1)
  }

  await makeConfigByGroupName(positionals[0], values.rootfs, values.ca)
}
